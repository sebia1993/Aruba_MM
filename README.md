...
Aruba MM Cleanup은 Aruba Mobility Master/MM에서 `profiling` Role 사용자 MAC을 조회하고, 설정한 삭제 대기 시간 후 자동으로 `aaa user delete mac <mac>`을 실행하는 Windows 11 운영 대시보드입니다.
...
6. `삭제 대기(초)`를 설정합니다. 기본값은 60초이고, 0초로 설정하면 조회 후 바로 삭제 단계로 진행합니다.
7. `장비 응답 대기(초)`는 SSH 접속, 인증, 명령 응답을 기다리는 시간입니다. 삭제 전 카운트다운 시간이 아닙니다.
8. `1회 실행`을 누르면 조회 후 `타이머` 카드에 삭제 대기가 표시됩니다.
9. 삭제 대기 중 중단하려면 `이번 삭제 취소`를 누릅니다.
10. 주기적으로 반복하려면 `주기(초)`를 설정하고 `주기 실행 시작`을 누릅니다. 주기 대기 시간도 같은 `타이머` 카드에 `다음 실행`으로 표시됩니다.
11. 장비 세션을 즉시 끊고 싶으면 `세션 연결 해제`를 누릅니다.
...
- 주기 실행 정지와 `이번 삭제 취소`는 삭제 대기 이후에도 다음 MAC 삭제 전과 검증 조회 전에 반영됩니다.
