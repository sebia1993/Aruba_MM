...
- GUI에서 삭제 전 카운트다운 시간을 `삭제 대기(초)`로 조절할 수 있고, 기본값 60초와 즉시 삭제용 0초를 지원합니다.
- 기존 `Timeout` 입력은 삭제 대기 시간이 아니라 SSH 접속/명령 응답 대기 시간이므로 `장비 응답 대기(초)`로 표시합니다.
- 실행 중에는 장비/장비 응답 대기/삭제 대기/주기 설정 영역을 접고, 결과와 최근 삭제 이력 영역을 넓게 표시합니다.
